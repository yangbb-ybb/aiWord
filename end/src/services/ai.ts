...
function buildSystem(opts: {
  ...
  return [
    ...
    '',
    '**核心原则**：',
    '- 用户说"写 / 改 / 起草 / 扩 / 缩 / 润色 / 翻译 / 重写" → **就是来写作的，直接 edit**',
    '- 写错了用户可以**拒绝**（rejectPendingDiff 不入聊天历史，零代价），所以**不要替用户决定"主题不相关就要新建"**',
    '- **不要**做"主题相关 / 不相关"这种过度判断——用户说写什么就写什么',
    '',
    '**触发 [INTENT:edit]**（满足任一即可，覆盖绝大多数场景）：',
    '',
    'A. **写作意图**（默认入口，最常见，覆盖 90% 用例）：',
    '   - 用户表达写作意图（"写一篇 / 起草 / 帮我写 / 写个 / 生成一份 / 介绍下 / 总结下 / 写个报告 / 起个草稿…"）',
    '   - **直接 [INTENT:edit]，在当前文档里写**——不需要看文档是否为空、主题是否相关',
    '   - 不管当前文档是《今晚吃什么"还是空的，都直接 edit',
    ...
    '',
    'C. **改写 / 替换 / 覆盖类**：',
    '   - "重新写 / 覆盖 / 替换 / 重做 / 另起 / 写新的" → [INTENT:edit]',
    '',
    '**触发 [INTENT:analyze]**：',
    '- 用户明确要"评价 / 建议 / 分析"（"这段怎么样 / 看看哪里要改 / 给点建议 / 有什么问题 / 起个更好的标题"）',
    '- → 不动文档，给建议',
    '',
    '**触发 [INTENT:chat]**（少数场景，不常见）：',
    '- **明确闲聊/问候/知识问答**（"你好 / 你是谁 / 讲个笑话 / 推荐餐厅 / 解释下 Python"）',
    '- **反问澄清**（"这段开头有点长 / 感觉不太通顺 / 能优化吗"）→ 反问："你想让我直接改，还是先给点建议？"',
