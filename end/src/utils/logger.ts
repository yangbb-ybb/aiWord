 ...
 *
 * **重要**:pino 的 timestamp 必须返回**带 `,"time":"..."` 前缀**的字符串,
 * pino 内部把它拼接到 `{"level":"..."` 后面。所以返回纯 ISO 串会让 JSON 残缺。
 * 参考 pino.stdTimeFunctions.isoTime 实现:
...
function beijingIsoTime(): string {
  ...
  return `,"time":"${beijing.toISOString().replace('Z', '+08:00')}"`
}
